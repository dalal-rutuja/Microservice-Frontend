...
// import { useLocation, useParams } from 'react-router-dom';
// import { useSidebar } from '../context/SidebarContext';
// import { AlertCircle, CheckCircle, X, Upload, Loader2, FileCheck } from 'lucide-react';
// import ApiService from '../services/api';
...
const AnalysisPage = () => {
      ...
      if (data.uploaded_files && Array.isArray(data.uploaded_files)) {
        data.uploaded_files.forEach((uploadedFile, index) => {
          ...
          if (uploadedFile.error) {
            ...
            setBatchUploads(prev => prev.map(upload =>
              upload.id === matchingUpload.id
                ? { ...upload, status: 'processing', fileId, progress: 20 }
                : upload
            ));
  ...
  const getProcessingStatus = async (file_id) => {
    try {
      ...
      setUploadedDocuments(prev => prev.map(doc => {
        if (doc.id === file_id) {
          const elapsedTime = (Date.now() - doc.startTime) / 1000;
          const estimatedTotal = progress > 0 ? (elapsedTime / progress) * 100 : null;
          const estimatedRemaining = estimatedTotal ? estimatedTotal - elapsedTime : null;
  ...
  const chatWithDocument = async (file_id, question, currentSessionId) => {
    try {
      ...
      if (error.message === 'Request cancelled') {
        console.log('[chatWithDocument] Request cancelled by user');
        return;
      }
      console.error('[chatWithDocument] Error:', error);
      setError(`Chat failed: ${error.message}`);
  ...
  const handleSend = async (e) => {
    ...
    if (isSecretPromptSelected) {
      ...
      try {
        ...
        if (error.message === 'Request cancelled') {
          console.log('[handleSend] Request cancelled by user');
          return;
        }
        console.error('[handleSend] Analysis error:', error);
        setError(`Analysis failed: ${error.message}`);
