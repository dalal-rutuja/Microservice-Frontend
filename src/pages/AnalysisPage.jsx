...
//           setProcessingStatus({ 
//             status: actualStatus, 
//             processing_progress: actualProgress 
//           });
...
//     if (isSecretPromptSelected) {
//       return `Add optional details for ${activeDropdown}...`;
//     }
...
//     if (processingStatus && processingStatus.status && 
//         processingStatus.status !== 'processed' && 
...
//                   className="flex-1 bg-transparent border-none outline-none text-gray-900 placeholder-gray-500 text-[15px] font-medium py-2 min-w-0 analysis-page-user-input"
//                   disabled={isLoading || isGeneratingInsights || !fileId || (processingStatus?.status !== 'processed' && processingStatus?.status !== null && progressPercentage < 100)}
//                 />
...
//                     placeholder={getInputPlaceholder()}
//                     className="flex-1 bg-transparent border-none outline-none text-gray-900 placeholder-gray-500 text-sm font-medium py-1 min-w-0 analysis-page-user-input"
//                     disabled={isLoading || isGeneratingInsights || !fileId || (processingStatus && processingStatus.status !== 'processed' && progressPercentage < 100)}
//                   />
...
//   ArrowRight, ChevronRight, AlertTriangle, Clock, Loader2,
//   Upload, Download, AlertCircle, CheckCircle, X, Eye, Quote, BookOpen, Copy,
//   ChevronDown, Paperclip, MessageSquare, FileCheck, Bot, Check, Circle
// } from 'lucide-react';

// const PROGRESS_STAGES = {
//   INIT: { range: [0, 15], label: 'Initialization' },
//   EXTRACT: { range: [15, 40], label: 'Text Extraction' },
//   CHUNK: { range: [40, 55], label: 'Chunking' },
//   EMBED: { range: [55, 75], label: 'Embeddings' },
//   STORE: { range: [75, 85], label: 'Database Storage' },
//   SUMMARY: { range: [85, 95], label: 'Summary Generation' },
...
//       <div className={`bg-white rounded-lg shadow-xl p-6 w-96 border-2 max-w-md transition-all duration-300 ${
//         isError ? 'border-red-200 animate-pulse' : isBatch ? 'border-yellow-200' : 'border-blue-200'
//       }`}>
...
//             <p className="text-red-700 text-sm mb-4 font-medium">{processingStatus.job_error || 'An error occurred during processing'}</p>
...
//                 <div
//                   className="bg-gradient-to-r from-[#21C1B6] to-blue-600 h-3 rounded-full transition-all duration-1000 ease-out relative overflow-hidden"
...
//             </div>
//             <div className="mb-4">
//               <p className="text-sm text-gray-700 font-medium bg-blue-50 p-2 rounded text-blue-800 break-words">
...
//                 return (
//                   <div key={key} className={`flex items-center space-x-3 py-1 transition-all duration-300 ${
//                     status === 'completed' ? 'opacity-100' : status === 'active' ? 'opacity-100' : 'opacity-50'
//                   }`}>
//                     {status === 'completed' ? (
//                       <Check className="h-4 w-4 text-green-500 animate-pulse" />
//                     ) : status === 'active' ? (
//                       <Loader2 className="h-4 w-4 text-[#21C1B6] animate-spin" />
//                     ) : (
//                       <Circle className="h-4 w-4 text-gray-300" />
//                     )}
//                     <span className={`text-xs font-medium transition-colors ${
//                       status === 'completed' ? 'text-green-600' : status === 'active' ? 'text-[#21C1B6] font-semibold' : 'text-gray-400'
//                     }`}>
//                       {label}
//                     </span>
//                   </div>
...
//     }
//     if (documentData && (processingStatus?.status === 'processing' || processingStatus?.status === 'batch_processing')) {
//       return `${processingStatus?.current_operation || (processingStatus?.status === 'batch_processing' ? 'Batch Processing...' : 'Processing document...')} (${Math.round(progressPercentage)}%)`;
//     }
//     // Show "Done" only when progress is exactly 100% (after animation completes)
//     if (documentData && processingStatus?.status === 'processed' && progressPercentage >= 100) {
//       return 'Done';
...
//                   <div
//                     className="bg-gradient-to-r from-[#21C1B6] to-blue-600 h-2 rounded-full transition-all duration-300 relative"
//                     style={{ width: `${currentProgressBarPercentage}%` }}
//                   >
//                     <div className="absolute inset-0 bg-white/20 animate-shimmer"></div>
//                   </div>
...
//                           {(doc.status === 'processing' || doc.status === 'batch_processing') && (
//                             <>
//                               <p className="text-xs text-[#21C1B6] mt-1 truncate font-medium">{doc.currentOperation}</p>
//                               <div className="w-full bg-gray-200 rounded-full h-1.5 mt-1 relative overflow-hidden">
//                                 <div
//                                   className="bg-gradient-to-r from-[#21C1B6] to-blue-600 h-1.5 rounded-full transition-all duration-300"
//                                   style={{ width: `${doc.processingProgress || 0}%` }}
//                                 >
//                                   <div className="absolute inset-0 bg-white/20 animate-shimmer"></div>
//                                 </div>
...
//               <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden relative">
//                 <div
//                   className={`h-3 rounded-full transition-all duration-1000 ease-out relative overflow-hidden bg-gradient-to-r ${getStageColor(progress)}`}
//                   style={{ width: `${progress}%` }}
//                 >
//                   <div className="absolute inset-0 bg-white/20 animate-shimmer"></div>
//                 </div>
...
//                 <div className="w-full bg-gray-200 rounded-full h-2 mt-2 relative overflow-hidden">
//                   <div
//                     className={`h-2 rounded-full transition-all duration-300 relative overflow-hidden bg-gradient-to-r ${getStageColor(currentProgressBarPercentage)}`}
//                     style={{ width: `${currentProgressBarPercentage}%` }}
//                   >
//                     <div className="absolute inset-0 bg-white/20 animate-shimmer"></div>
//                   </div>
...
//                               <div className="w-full bg-gray-200 rounded-full h-1.5 mt-1 relative overflow-hidden">
//                                 <div
//                                   className={`h-1.5 rounded-full transition-all duration-300 bg-gradient-to-r ${getStageColor(doc.processingProgress || 0)}`}
//                                   style={{ width: `${doc.processingProgress || 0}%` }}
//                                 >
//                                   <div className="absolute inset-0 bg-white/20 animate-shimmer"></div>
//                                 </div>
...
//         return progress < 50 ? `Processing... (${Math.round(progress)}%)` : progress < 90 ? `Analyzing... (${Math.round(progress)}%)` : `Finalizing... (${Math.round(progress)}%)`;
//       case 'batch_processing':
//         if (progress >= 100) return 'Done';
//         return progress < 30 ? `Batch Processing... (${Math.round(progress)}%)` : progress < 70 ? `Processing Documents... (${Math.round(progress)}%)` : progress < 95 ? `Analyzing Batch... (${Math.round(progress)}%)` : `Completing... (${Math.round(progress)}%)`;
//       case 'processed':
//         return progress >= 100 ? 'Done' : 'Processing...';
//       case 'error':
//       case 'failed':
...
//     <div className="fixed top-4 left-1/2 z-50 transform -translate-x-1/2">
//       <div
//         className={`bg-white rounded-lg shadow-xl p-6 w-96 border-2 max-w-md transition-all duration-300 ${
//           isError
...
//               <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden relative">
//                 <div
//                   className={`h-3 rounded-full transition-all duration-1000 ease-out relative overflow-hidden bg-gradient-to-r ${getStageColor(
//                     progress
//                   )}`}
//                   style={{ width: `${progress}%` }}
//                 >
...
//                               <div className="w-full bg-gray-200 rounded-full h-1.5 mt-1 relative overflow-hidden">
//                                 <div
//                                   className={`h-1.5 rounded-full transition-all duration-300 bg-gradient-to-r ${getStageColor(
...
//                           </div>
//                         </div>
//                         {selectedMessageId === msg.id && <ChevronRight className="h-4 w-4 text-[#21C1B6] flex-shrink-0 ml-2" />}
//                       </div>
//                     </div>
//                   ))}
//                 {messages.length > displayLimit && !showAllChats && (
